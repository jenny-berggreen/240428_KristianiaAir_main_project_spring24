...
const fetchAndCheckCountries = async (value, cityDetails, departureDetails, returnDetails, travelersInput, totalDetails) => {
    try {
        const response = await fetch('https://restcountries.com/v3.1/all');
        const countries = await response.json();
        checkValue(value, countries, cityDetails, departureDetails, returnDetails, travelersInput, totalDetails);
    } catch (error) {
        console.log(error);
    }
};
...
const checkValue = (value, countries, cityDetails, departureDetails, returnDetails, travelersInput, totalDetails) => {
    ...
    if(value) {
        ...
        if(!match) {
            ...
            requiredSpan.textContent = ' Invalid country name!';
            requiredSpan.style.color = 'red';
            countryLabel.append(requiredSpan);
        }
    }
};
